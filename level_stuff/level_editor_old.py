...
GUI that lets you edit each individual chatacter on the vic-20 screen. VERY useful for making the titlescreen. Will be fixed 
...
def export_level_data():
    ...
    if level_name:
        with open(f"./text_levels/{level_name}.bin", "w") as f:
            # write binary data to a binary file in the format of:
            # 1 byte for code of character in level
            pass
